...
CSV.foreach('../csv/FRD.csv', headers: true) do |row|
  date = row['Date']
  stock3_volume = row['Adj Close'].to_i
  index = data.index { |d| d[0] == date }
  if index
...
CSV.foreach('../csv/TGB.csv', headers: true) do |row|
  date = row['Date']
  stock4_volume = row['Adj Close'].to_i
  index = data.index { |d| d[0] == date }
  if index
...
CSV.foreach('../csv/UHS.csv', headers: true) do |row|
  date = row['Date']
  stock5_volume = row['Adj Close'].to_i
  index = data.index { |d| d[0] == date }
  if index
...
CSV.open('../csv/adj_close.csv', 'w', headers: ['Date', 'COF', 'DTST', 'FRD', 'TGB', 'TKR', 'UHS']) do |csv|
  # Write the headers
  csv << csv.headers
  # Write the data rows
  data.each do |row|
