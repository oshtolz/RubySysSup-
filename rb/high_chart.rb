...
CSV.foreach(csv_path, headers: true) do |row|
  data << [row['Date'], row['High']]
end

# Filter data
data = data.select { |row| row[0].start_with?('2021', '2022', '2023') && row[1] != 'null' }
...
# Set data for chart
g.data('High', data.map { |row| row[1].to_f })
...
output_file = "#{output_folder}/#{input_file.chomp('.csv').upcase}_High_2021-2023.png"
